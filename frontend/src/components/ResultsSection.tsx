...
import { Layers, SearchX, Clock, Zap, Play, CheckCircle2, Loader2, ShieldCheck } from 'lucide-react';
    ...
    <div className="card results-card">
        ...
        {results.length > 0 && !isSearching && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ fontSize: '0.75rem', color: '#10b981', display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}>
              <ShieldCheck size={14} /> Color/Category Verified
            </span>
            <span style={{ fontSize: '0.8125rem', color: '#94a3b8' }}>
              Displaying Top {results.length} frame matches for &quot;{activeQuery}&quot; (Threshold ≥ 0.24)
            </span>
          </div>
        )}
      </div>
      ...
      {isSearching ? (
        <div className="empty-results">
          ...
          <h3 className="empty-results-title">Computing CLIP Prompt Ensembling & Contrastive Filtering...</h3>
          <p className="empty-results-desc">
            Ensembling prompt templates and verifying color/category contrast in memory.
          </p>
        </div>
      ) : results.length > 0 ? (
        <div
        ...
        <div className="empty-results">
          ...
          <h3 className="empty-results-title">
            {activeQuery ? 'No Accurate Matches Exceeding Threshold' : 'No Search Results'}
          </h3>
          <p className="empty-results-desc">
            {activeQuery
              ? `Contrastive verification filtered out false matches for "${activeQuery}". Try descriptive queries like "person wearing red shirt" or "black dog".`
              : 'Enter a search term above (e.g., "person wearing red shirt", "black dog", "car", "phone") to display verified frame matches.'}
          </p>
        </div>
      )}
    </div>
